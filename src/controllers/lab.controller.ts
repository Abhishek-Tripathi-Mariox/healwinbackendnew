...
/**
 * Diagnostic lab platform — admin management (CRUD + approve/reject) and the
 * public Lab Locator (listing + geo filter + public onboarding submission).
 *
 * Mirrors pharmacy.controller.ts one-for-one so the two locators behave
 * identically; keep them in step when either changes.
 */
...
export const adminUpdate = async (
  ...
  const doc = buildLabFromBody({ ...lab.toObject(), ...b });
  Object.assign(lab, doc);
  if (b.isActive !== undefined)
    lab.isActive = b.isActive === true || b.isActive === "true";
  if (req.file) {
    const { images } = await uploadFileToAws([req.file]);
    lab.image = images as string;
  }
  await lab.save();
  req.rData = { lab };
  req.msg = "lab_updated";
...
export const adminApprove = async (
  ...
  lab.status = approve ? "approved" : "rejected";
  if (!approve) lab.rejectionReason = req.body?.reason || undefined;
  await lab.save();
  req.rData = { lab };
  req.msg = "lab_updated";
  return next();
...
/**
 * GET /labs — public locator. Approved + active only. Supports
...
export const publicList = async (
  ...
  if (coords) {
    ...
    items = await Lab.find(query).limit(100).lean();
  } else {
    items = await Lab.find(query)
...
/** POST /labs/request — public onboarding submission (status=pending). */
export const publicSubmit = async (
  ...
  req.rData = { labId: lab._id };
  req.msg = "lab_submitted";
  return next();
};
